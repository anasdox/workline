...
CREATE TABLE IF NOT EXISTS tasks(
  ...
  type TEXT CHECK(type IN ('technical','feature','bug','docs','chore')) NOT NULL,
  title TEXT NOT NULL,
  description TEXT,
  status TEXT CHECK(status IN ('planned','in_progress','review','done','rejected','canceled')) NOT NULL,
  assignee_id TEXT,
  work_proof_json TEXT,
  validation_mode TEXT CHECK(validation_mode IN ('none','all','any','threshold')) NOT NULL DEFAULT 'none',
